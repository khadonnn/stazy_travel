...
const AboutPageContent: React.FC = () => {
  const { aboutData, isLoading, loadData } = useAbout();
  useEffect(() => {
    // Nếu chưa có dữ liệu thì mới gọi load
    if (!aboutData) {
      loadData();
    }
  }, [aboutData, loadData]);
  ...
  return (
    <div className="min-h-screen w-full flex items-center mx-auto bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      <motion.div
        ...
        <motion.section className="mb-20" variants={itemVariants}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {services.items.map((service, index) => {
              const IconComponent = serviceIcons[service.id] || Hotel;
              return (
                <motion.div
                  key={service.id}
        ...
        <motion.section className="mb-20" variants={itemVariants}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {values.items.map((value) => {
              ...
              const colorClass = valueColors[value.color] || valueColors.blue;
